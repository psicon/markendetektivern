...
function Search({ onOpenProduct, onBack, variant = 'a' }) {
  ...
  return (
    <div style={{ fontFamily: F, background: 'var(--th-bg)', minHeight: '100%', paddingBottom: 110, position: 'relative' }}>
      {/* Eigenmarken / Marken pill toggle */}
      <div style={{
        margin: '12px 20px 0', height: 40, borderRadius: 20, background: 'var(--th-low)',
      ...
      <div style={{
        ...
        background: 'color-mix(in srgb, var(--th-bg) 78%, transparent)',
        ...
        <div style={{ padding: '0 20px' }}>
          <div style={{
            height: 38, background: 'color-mix(in srgb, var(--th-card) 92%, transparent)', borderRadius: 11,
        ...
        <div style={{
          ...
          {/* Divider */}
          <div style={{ flexShrink: 0, width: 1, background: 'var(--th-border-md)', margin: '4px 4px' }}/>
          {anyFilter && (
            <FilterChip
...
function FilterChip({ label, value, onClick, onClear, muted, icon, strong }) {
  ...
  return (
    <div style={{
      ...
      background: bg,
      border: active || muted ? '1px solid transparent' : '1px solid var(--th-border-md)',
      color: fg,
      font: `600 12px ${F}`,
        ...
        {INGREDIENTS.map(x => {
          ...
          return (
            <button key={x} onClick={() => onToggle(x)} style={{
              ...
              border: on ? '1px solid transparent' : '1px solid var(--th-border-strong)',
...
function BrandCard({ product, onClick }) {
  ...
  return (
    <button onClick={onClick} style={{
      width: '100%', background: 'var(--th-card)', borderRadius: 16, overflow: 'hidden',
      border: 0, padding: 0,
