  ...
  return (
    <div style={{ fontFamily: F, background: '#F9FAFB', minHeight: '100%', paddingBottom: 110 }}>
      ...
      <div style={{
        ...
        background: '#F9FAFB', zIndex: 5, position: 'sticky', top: 0,
      }}>
        ...
        <div style={{ position: 'relative', borderRadius: 20, overflow: 'hidden' }}>
          ...
          <div style={{
            ...
            <button onClick={()=>setOnList(v=>!v)} style={{
              ...
              background: onList ? COLORS.primary : '#fff',
              boxShadow: '0 2px 6px rgba(0,0,0,.08)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>
        ...
        {sorted.map((nn, i) => {
          ...
          return (
            <div key={nn.id} onClick={() => setIdx(i)} style={{
              ...
              background: '#fff', borderRadius: 18, boxShadow: SH_MD, overflow: 'hidden',
              ...
              {/* Divider */}
              <div style={{ height: 1, background: 'rgba(25,28,29,.08)', margin: '0 14px' }}/>
              {/* Price + actions row */}
          ...
          <>
            ...
            {getIngredients(p).map(([name, matchPct], i) => {
              ...
              <div key={i} style={{
                ...
                borderBottom: i < 2 ? '1px solid rgba(25,28,29,.05)' : 'none',
              }}>
                ...
                <div style={{ display: 'flex', gap: 3, justifyContent: 'flex-end' }}>
                  {[1,2,3,4,5].map(n => (
                    <span key={n} style={{
                      ...
                      background: n <= Math.round(adj / 20) ? COLORS.primary : 'rgba(25,28,29,.1)',
                    }}/>
                  ))}
                </div>
              </div>
          ...
          {alternatives.map(ap => {
            ...
            return (
              <button key={ap.id} onClick={() => onOpenProduct && onOpenProduct(ap)} style={{
                width: 140, flexShrink: 0, background: '#fff', borderRadius: 14, padding: 0, overflow: 'hidden',
...
function ProductDetailOrphan({ product, onBack, onOpenProduct, onOpenLists }) {
  ...
  return (
    <div style={{ fontFamily: F, background: '#F9FAFB', minHeight: '100%', paddingBottom: 110 }}>
      ...
      <div style={{ margin: '24px 20px 0', background: '#fff', borderRadius: 16, padding: 16, boxShadow: SH_SM }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingBottom: 12, borderBottom: '1px solid rgba(25,28,29,.06)' }}>
      ...
      <div style={{ margin: '18px 20px 0' }}>
        {tab === 'ingredients' ? (
          <div style={{ background: '#fff', borderRadius: 14, padding: '6px 16px', boxShadow: SH_SM }}>
            {getOrphanIngredients(p).map(([name, note], i, arr) => (
              <div key={i} style={{
                display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                padding: '12px 0',
                borderBottom: i < arr.length - 1 ? '1px solid rgba(25,28,29,.06)' : 'none',
              }}>
                <span style={{ font: `500 14px ${F}`, color: COLORS.text }}>{name}</span>
