...
const COLORS = {
  primary: '#0d8575', primaryHover: '#0a6f62', secondary: '#42a968',
  surface: '#f8fafb', low: '#f2f4f5', high: '#e7eaeb', card: '#ffffff',
  text: '#191c1d', textVar: '#43484a', muted: '#6c7275', faint: '#a0a5a8',
  stufe: { 1:'#ef2d1a', 2:'#f5720e', 3:'#fbc801', 4:'#73c928', 5:'#0d8575' },
  warn: '#ff9500', err: '#ff3b30',
  gradient: 'linear-gradient(135deg,#0d8575 0%,#0aa18a 100%)',
};
const F = "'Nunito',-apple-system,BlinkMacSystemFont,system-ui,sans-serif";
const SH_SM = '0 1px 3px rgba(25,28,29,.04)';
const SH_MD = '0 2px 8px rgba(25,28,29,.05)';
const SH_LG = '0 12px 24px rgba(25,28,29,.06)';
    ...
    <div style={{ display: 'inline-flex', gap: 2, alignItems: 'center' }}>
      {[1,2,3,4,5].map(n => (
        <span key={n} style={{
          ...
          background: n <= stufe ? color : 'rgba(25,28,29,.12)',
        }}/>
      ))}
    </div>
  );
...
function StufenBar({ stufe, style = {} }) {
  return (
    <div style={{ display: 'flex', height: 6, borderRadius: 999, overflow: 'hidden', gap: 2, ...style }}>
      {[1,2,3,4,5].map(n => (
        <div key={n} style={{ flex: 1, background: n <= stufe ? COLORS.stufe[stufe] : 'rgba(25,28,29,.08)', borderRadius: 999 }}/>
      ))}
    </div>
  );
}
...
function ProductCard({ product, noName, variant = 'grid', onClick }) {
  ...
  return (
    <button onClick={onClick} style={{
      width, background: '#fff', borderRadius: 16, padding: 0, overflow: 'hidden',
...
function TabBar({ active = 'home', onNav }) {
  ...
  return (
    <div style={{
      ...
      background: '#fff', borderTop: '1px solid rgba(25,28,29,.06)', height: 82,
