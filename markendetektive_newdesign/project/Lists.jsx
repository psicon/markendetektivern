...
function ShoppingList({ onBack, onOpenHistory }) {
  ...
  return (
    <div style={{
      ...
      {/* Tabs */}
      <div style={{ background: 'var(--th-card)', display: 'flex', flexShrink: 0, boxShadow: '0 1px 0 rgba(25,28,29,.06)' }}>
        {[
  ...
  return (
    <div style={{
      ...
      {expanded && p.noNames?.length > 0 && (
        <div style={{ background: '#fafafa', padding: '4px 10px 12px', borderTop: '1px solid var(--th-border)' }}>
          ...
          {p.noNames.map(nn => {
            ...
            return (
              <div key={nn.id} onClick={() => setSelectedAlt(nn.id)} style={{
                background: 'var(--th-card)', borderRadius: 10, padding: 8, marginBottom: 6,
  ...
  if (item.manual) {
    return (
      <div style={{
        ...
        border: '1px solid var(--th-border)', display: 'flex', alignItems: 'center', gap: 10,
      }}>
        <div style={{
          width: 62, height: 62, borderRadius: 10, flexShrink: 0,
  ...
  return (
    <div
      ...
      <button onClick={onDec} style={{
        width: 28, height: 28, borderRadius: 14, border: 0, background: 'var(--th-card)', cursor: 'pointer',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
      }}>
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        background: 'var(--th-card)', width: '100%', borderTopLeftRadius: 18, borderTopRightRadius: 18,
                ...
                return (
                  <button key={o.v} onClick={() => setNFilter(f => ({ ...f, stufe: o.v }))}
                    style={{
                      ...
                      color: on ? 'var(--th-card)' : LIST_C.text,
              ...
              {[{ v: 'any', name: 'Alle' }, ...Object.entries(window.MARKETS || {}).map(([k, v]) => ({ v: k, name: v.name, tint: v.tint }))].map(o => {
                ...
                return (
                  <button key={o.v} onClick={() => setNFilter(f => ({ ...f, market: o.v }))}
                    ...
                    {o.name}
                    {isFav && <MdI name="heart" size={10} color={on ? 'var(--th-card)' : '#ef4444'}/>}
                  </button>
                );
              })}
            </div>
...
function ToggleRow({ label, on, onChange }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid var(--th-border)' }}>
      ...
      <button onClick={() => onChange(!on)} style={{
        ...
        <div style={{
          ...
          width: 20, height: 20, borderRadius: 10, background: 'var(--th-card)',
          transition: 'left .15s',
        }}/>
      </button>
    </div>
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding: '4px 18px 0' }}>
          ...
          <input
            ...
            style={{
              width: '100%', border: '1px solid var(--th-border-strong)', borderRadius: 10,
            ...
            {MANUAL_ICONS.map(i => {
              ...
              return (
                <button key={i} onClick={() => setIcon(i)} style={{
                  ...
                  background: on ? LIST_C.primary : '#f0f2f3',
                  border: on ? 'none' : '1px solid var(--th-border)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>
                  <MdI name={i} size={22} color={on ? 'var(--th-card)' : LIST_C.text}/>
                </button>
              );
            })}
          </div>
        ...
        <div style={{ padding: '18px 18px 0', display: 'flex', gap: 8 }}>
          <button onClick={onClose} style={{
            ...
            border: '1px solid var(--th-border-strong)', font: `700 13px ${LIST_F}`,
          }}>Abbrechen</button>
          ...
          return (
            <div key={label} style={{ marginBottom: 14 }}>
              <div style={{
                ...
                <div style={{ flex: 1, height: 1, background: 'var(--th-border-md)' }}/>
...
function HistoryRow({ h }) {
  ...
  return (
    <div style={{
      background: 'var(--th-card)', borderRadius: 12, padding: 10, marginBottom: 8,
      border: '1px solid var(--th-border)', display: 'flex', alignItems: 'center', gap: 10,
    }}>
      {visual}
      <div style={{ flex: 1, minWidth: 0 }}>
